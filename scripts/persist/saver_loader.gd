...
func save_game(address: String = "user://savegame.tres"):
	...
	player_platformer_serializable_component.on_save_game(saved_data)
	
	GameManagerSingleton.get_tree().call_group("Serializable", "on_save_game", saved_data)
...
func load_game(address: String = "user://savegame.tres"):
	...
	for sd in saved_game.saved_datas:
		if sd is PlayerPlatformerSavedData:
			...
			player_platformer_serializable_component.on_save_game(ppsd)
